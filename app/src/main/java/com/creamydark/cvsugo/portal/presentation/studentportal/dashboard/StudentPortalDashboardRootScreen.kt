package com.creamydark.cvsugo.portal.presentation.studentportal.dashboard
...
import com.creamydark.cvsugo.portal.presentation.studentportal.dashboard.viewmodel.StudentPortalViewModel

@Composable
fun StudentPortalDashboardRootScreen(
    modifier: Modifier = Modifier,
    viewModel: StudentPortalViewModel = hiltViewModel()
) {
    if(viewModel.state.authenticationState == AuthenticationState.Authenticated){
        StudentPortalDashboardScreen(modifier = modifier,viewModel.state)
    }else{
...
fun StudentPortalDashboardScreen(
    modifier: Modifier = Modifier,
    state: StudentPortalDashboardScreenState,
) {
    ...
    LazyColumn(modifier = modifier
        ...
        item {
            InfoCardWithButton(
                ...
                label = "Status",
                value = if (!state.academicInformation.enrolled)"Not Enrolled" else "Enrolled"
            ){
                Button(
                    onClick = {
            ...
            StudentDashboard(
                academicInformation = state.academicInformation
            )
            Spacer(modifier = Modifier.height(22.dp))
        }

        items(items = state.subjectEnrolledList, key = { it.subjectName }) {
            data ->
            SubjectsListItem(
